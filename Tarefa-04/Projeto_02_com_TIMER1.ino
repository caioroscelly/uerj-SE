...
void setup(void) 
{
  ...
  ////################# TIMER 1 = DELAY - PARA ATUALIZAR TELA OLED########################
 // estava fazendo testes com o led do arduino, favor ignorar 
 pinMode(ledPin, OUTPUT);
    // initialize Timer1
  TCCR1A = 0x00;                            //confiurra timer para operação normal pinos OC1A e OC1B desconectados //Timer1 Control Reg A: Normal port operation, Wave Gen Mode normal
  TCCR1B = 0x00;                           //limpa registrador //Disable Timer1 while we set it up
  TCCR1B |= (1 << CS10) | (1 << CS12);  // prescale = configura prescaler para 1024: CS12 = 1 e CS10 = 1
  TCNT1 = 0x48E4;                       // incia timer com valor para que estouro ocorra em 1 segundo
                                        // PARA TIMER1 = 1 SEGUNDO -- 2^16 -- 65536-(16MHz/1024/1Hz) = 49911 = 0xC2F7  
                                        // PARA TIMER = 3 SEGUNDOS -- 2^16 --  65536-(16MHz/1024/0.3333Hz) = 18660 = 0x48E4                  
  TIMSK1 = 0X00;                        //Desabilita int timer 1 //Timer1 INT Reg: Timer2 Overflow Interrupt Enable
...
if ( (millis() - delayIntervalo) > intervalo ) {
  ...
  delay(50);
   ...
   //SUBSTITUINDO FUNCAO delay que atualiza a tela oled a cada 1 segundo pelo TIMER1
  //delay(1000);// ATUALIZA O DISPLAY A CADA 5000ms
   while(!TIFR1) {}
 //overflow has occurred! -- ocorreu um overflow, que conforme foi setado, ocorre a cada 1 segundo
 //TIFR1 = 0x00; //Clear Timer Overflow Flag
 //TCNT1  = 1; //Reset Timer to 0
  TIFR1 = 0b00000001; //Clear Timer Overflow Flag
  TCNT1  = 0x48E4; //Reset Timer to 0
